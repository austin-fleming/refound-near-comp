...
async function start() {
  ...
  console.log("Required deposit: ", formatNearAmount(requiredDeposit));
  try {
  ...
  try {
    let dropId = await getRecentDropId(
      fundingAccount,
      FUNDING_ACCOUNT_ID,
      KEYPOM_CONTRACT
    );
  ...
  for (var i = 0; i < keyPairs.length; i++) {
    ...
    ] = `https://testnet.mynearwallet.com/linkdrop/${KEYPOM_CONTRACT}/${keyPairs[i].secretKey}`;
    console.log(
      `https://testnet.mynearwallet.com/linkdrop/${KEYPOM_CONTRACT}/${keyPairs[i].secretKey}`
    );
  }
