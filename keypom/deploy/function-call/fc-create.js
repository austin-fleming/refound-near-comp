...
let FUNDING_ACCOUNT_ID = "slyracoon-nonledger.testnet";
let LINKDROP_NEAR_AMOUNT = "0.1";
...
async function start() {
  ...
  if (!LINKDROP_PROXY_CONTRACT_ID) {
    const dev_account = await readFile(
      `/home/slyracoon23/Documents/metabuild/refound-near-comp/keypom/neardev/dev-account`
    );
  ...
  console.log(
    `initializing contract for account ${LINKDROP_PROXY_CONTRACT_ID}`
  );
  try {
  ...
  try {
    ...
    await writeFile(
      path.resolve(__dirname, `views-create.json`),
  ...
  for (var i = 0; i < keyPairs.length; i++) {
    curPks[
      keyPairs[i].publicKey.toString()
    ] = `https://wallet.testnet.near.org/linkdrop/${LINKDROP_PROXY_CONTRACT_ID}/${keyPairs[i].secretKey}`;
    console.log(
      `https://wallet.testnet.near.org/linkdrop/${LINKDROP_PROXY_CONTRACT_ID}/${keyPairs[i].secretKey}`
    );
