...
const estimateRequiredDeposit = async (
  ...
  let { numNoneFcs, depositRequiredForFcDrops } = getNoneFcsAndDepositRequired(
    fcData,
    usesPerKey
  );
...
const getFtCosts = async (near, numKeys, usesPerKey, ftContract) => {
  ...
  const storageBalanceBounds = await viewAccount.viewFunction(
    ftContract,
    "storage_balance_bounds",
    {}
  );
  console.log("storageBalanceBounds: ", storageBalanceBounds);
  let costs = new BN(storageBalanceBounds.min)
    .mul(new BN(numKeys))
    .mul(new BN(usesPerKey))
    .add(new BN(storageBalanceBounds.min));
  console.log("costs: ", costs.toString());
  return costs.toString();
};

// Estimate the amount of allowance required for a given attached gas.
const getRecentDropId = async (
  fundingAccountObject,
  accountId,
  keypomContract
) => {
  ...
  let dropsForOwner = await fundingAccountObject.viewFunction(
    keypomContract,
    "get_drops_for_owner",
    { account_id: accountId, from_index: (dropSupplyForOwner - 1).toString() }
  );
