...
impl Contract {
    #[payable]
    pub fn create_factory_subaccount_and_deploy_nft_series(
        &mut self,
        name: String,
        // INIT ARGS
        owner_id: AccountId,
        ...
        let contract_bytes = self.nft_series_code.len() as u128;
        ...
        let init_args =
            near_sdk::serde_json::to_vec(&NFTSeriesInitArgs { owner_id, metadata }).unwrap();

        let mut promise = Promise::new(subaccount.clone())
            .create_account()
            .transfer(attached)
            .deploy_contract(self.nft_series_code.clone())
        ...
        promise.then(
            Self::ext(env::current_account_id())
                .create_factory_subaccount_and_deploy_nft_series_callback(
                    subaccount,
    ...
    #[private]
    pub fn create_factory_subaccount_and_deploy_nft_series_callback(
        &mut self,
        account: AccountId,
        user: AccountId,
        attached: Balance,
